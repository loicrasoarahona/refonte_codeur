    ...
    $('#reset-user-form').on('click', '#send_complete', function(e) {
        ...
        if (msgError == '') {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.indexOf('<?php echo $base_url ?>') > -1) {
                        alertError('Votre mot de passe a été modifié. Connecter-vous.',
                            'success');
                        setTimeout(function() {
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('.message-notif .nb-notif').removeClass(
        ...
        if (msgError == '') {
            ...
            $.ajax({
                ...
                success: function(data_1) {
                    $('#msgbox-modal #show-message-box').html(
                        '<div class="m-3">Chargement message ...</div>');
    ...
    $('.filtrable-table').on('click', '.nav-link', function() {
        ...
        if ($(this).data('target') != "all") {
            $('table.list-filtrable-table .ln-filtrable').filter(':not(.' + $(this).data('target') +
                ')').hide();
    ...
    $('#add-user-form .submit-add').on('click', function(e) {
        ...
        if (nameuser.val().length < 4) {
            ...
            nameuser.parent().find('.invalid-feedback').html(
                "<i class='icon-warning-sign'></i> Veuillez saisir votre nom et prénom.");
            rep_nameuser = false;
        } else {
        ...
        if (loginuser.val().length < 7) {
            loginuser.addClass('is-invalid');
            loginuser.parent().find('.invalid-feedback').html(
                "<i class='icon-warning-sign'></i> Veuillez saisir un nom d'utilisateur valide.");
            rep_loginuser = false;
        } else {
            loginuser.removeClass('is-invalid');
            rep_loginuser = true;
        ...
        if (emailuser.val().length < 8) {
            emailuser.addClass('is-invalid');
            emailuser.parent().find('.invalid-feedback').html(
                "<i class='icon-warning-sign'></i> Veuillez saisir votre E-mail.");
            rep_emailuser = false;
        } else {
            emailuser.removeClass('is-invalid');
            rep_emailuser = true;
    ...
    $('#add-ticket-user-form .submit-add').on('click', function(e) {
        ...
        if (sujet.val().length < 7) {
            sujet.addClass('is-invalid');
            sujet.parent().find('.invalid-feedback').html(
                "<i class='icon-warning-sign'></i> Veuillez saisir un sujet valide.");
            rep_sujet = false;
        } else {
            sujet.removeClass('is-invalid');
            rep_sujet = true;
