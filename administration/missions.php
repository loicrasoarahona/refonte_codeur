...
<nav class="navbar navbar-expand navbar-light bg-light mb-4">
    ...
    <ul class="navbar-nav ml-auto">
        <li class="nav-item">
            <a href="<?= url_panel; ?>/missions.php?action=add" class="button btn-primary btn-icon-split">Ajouter un
                offre</a>
        </li>
    </ul>
</nav>
...
<div class="transaction-list">
    <form method="post">
        ...
        <?php
                  ...
                  // var_dump($dones_offers);
                    if ($dones_offers['actif'] == 1) { 
                        $dones_offers['actif'] = 'Actif'; 
                        $boutonActif = '<a href="" class="m-r-5"><div class="display-inline-block button bg-red bg-red-hover color-white p-5-10 b-r-50 uppercase">Mettre en pause</div></a>'; 
                    } else if ($dones_offers['actif'] == 0) {
                        $dones_offers['actif'] = 'Inactif'; 
                        $boutonActif = '<a href="" class="m-r-5"><div class="display-inline-block button bg-green bg-green-hover color-white p-5-10 b-r-50 uppercase">Activer</div></a>'; }

                    if ($dones_offers['actif'] == "Actif") {
                        $dones_offers['actif'] = 'Actif';
                        $boutonActif = '<button name="desactive" value="'.$dones_offers['id'].'" style="margin:0.75vh 0.5vh" class="btn btn-warning ">Pause</button>';
                    } else if ($dones_offers['actif'] == "Inactif") {
                        $dones_offers['actif'] = 'Inactif';
                        $boutonActif = '<button name="active" value="'.$dones_offers['id'].'" style="margin:0.75vh 0.5vh" class="btn btn-warning ">Activer</button>';
                    }
                    ?>
        ...
        <div class="transaction-list-item" style="overflow: hidden;">
            ...
            <div class="transaction-list-item-price flexy">
                <?= $boutonActif; ?>
                <a href="<?= url_panel; ?>/missions.php?action=update&id=<?= $dones_offers['id']; ?>"
                    style="margin:0.75vh 0.5vh" class="btn btn-primary">Modifier</a>
            </div>
        </div>
        <!-- /TRANSACTION LIST ITEM -->
        <?php endforeach; ?>
...
<div class="form-box-items">
    ...
    <div class="form-box-item">
        ...
        <form id="profile-info-form" method="post" enctype="multipart/form-data">
            ...
            <div class="input-container ">
                ...
                <input type="text" id="nom" name="nom" value="<?= $post_nom; ?>"
                    placeholder="Entrez le nom de l'offre..." required>
            </div>
            <!-- /INPUT CONTAINER -->
            ...
            <div class="input-container">
                <label for="" class="text-primary">Catégorie</label>
                ...
                <input type="text" id="nom_categorie" name="nom_categorie" placeholder="Ou en créer une nouvelle">
            </div>
            <script>
            ...
            <div class="input-container">
                ...
                <input type="text" id="description1" name="description"
                    placeholder="Entrez une courte description (optionel)" value="<?= $post_description; ?>">
            </div>
            <!-- /INPUT CONTAINER -->
            ...
            <div class="input-container ">
                ...
                <input type="number" step="0.01" min="0.01" id="remuneration" name="remuneration" placeholder="0,01"
                    value="<?= $post_remuneration; ?>" required>
            </div>
            <!-- /INPUT CONTAINER -->
            ...
            <div class="input-container ">
                ...
                <input type="number" min="0" max="1" id="valid" name="valid" placeholder="0" value="<?= $post_valid; ?>"
                    required>
            </div>
            <!-- /INPUT CONTAINER -->
            <div class="input-container ">
                ...
                <div class="input-container ">
                    ...
                    <input type="number" step="0.01" min="0.01" id="montant" name="montant" placeholder="0,01" value=""
                        required>
                </div>
                <!-- /INPUT CONTAINER -->
                ...
                <div class="input-container">
                    ...
                    <input id="image" name="imageMission" type="file" placeholder="Uploader une image" required
                        style="width: 100%;padding: 10px;border: 1px solid #ebebeb;">
                </div>
                <!-- /INPUT CONTAINER -->
                <input type="hidden" name="idoffre" value="new" />
                <input type="submit" name="submit_add" class="button  btn-primary btn-icon-split"
                    value="Ajouter la mission">
        </form>
        <!-- DASHBOARD CONTENT -->
        <?php elseif ($_GET['action'] == 'update'):?>
        <?php //Bloc req SQL pour le formulaire de modification
            $sqlMissions = $pdo->query("SELECT offers.*, group_offers.nom as nom_categorie FROM offers join group_offers on id_group=group_offers.id WHERE offers.id = '".intval($_GET['id'])."'");
            $resultatMissions = $sqlMissions->fetch();
            $nomOffre = $resultatMissions['nom'];
            $categorie_id = $resultatMissions['id_group'];
        ...
        <form id="profile-info-form" method="post" enctype="multipart/form-data">
            <!-- INPUT CONTAINER -->
            <label for="nom" class="text-primary">Nom de l'offre</label>
            <input type="text" id="nom" name="nom" value="<?= $nomOffre; ?>" placeholder="Entrez le nom de l'offre..."
                required>
            <!-- /INPUT CONTAINER -->
            ...
            <label for="remuneration" class="text-primary">Rémunération aux membres</label>
            <input type="number" step="0.01" min="0.01" id="remuneration" name="remuneration" placeholder="0,01"
                value="<?= $remunerationOffre; ?>" required>
            <!-- /INPUT CONTAINER -->
            ...
            <!-- INPUT CONTAINER -->
            <label for="valid" class="text-primary">Validation directe (0 = non | 1 = oui)</label>
            <input type="number" min="0" max="1" id="valid" name="valid" placeholder="0" value="<?= $validOffre; ?>"
                required>
            <!-- /INPUT CONTAINER -->
            ...
            <!-- INPUT CONTAINER -->
            <label for="imageMission" class="text-primary">Image de la mission</label>
            <input id="image" name="imageMission" type="file" placeholder="Uploader une image"
                style="width: 100%;padding: 10px;border: 1px solid #ebebeb;">
            <!-- /INPUT CONTAINER -->

            <!-- /INPUT CONTAINER -->
            <input type="submit" name="submit_upd" class="button btn-primary btn-icon-split"
                value="Modifier la mission">
        </form>
        <!-- /FORM BOX ITEM -->
        <!-- DASHBOARD CONTENT -->
        <?php endif; ?>
